...
st.markdown("""
<style>
    /* Anthropic Warm Light Theme - Default */
    :root {
        --background: #F7F0E8;
        --bg: #F7F0E8;
        --surface1: #EFE6DA;
...
try:
    ...
    st.error("ERROR: Could not import custom model. Ensure `ME_FPN_model.py` exists.")
...
def load_segmentation_model(path):
    ...
    try:
        ...
        model = MyCustomFPN(encoder_name="resnet34", encoder_weights=None, classes=1, in_channels=3)
...
def postprocess_segmentation(output_mask, original_crop_shape, seg_threshold):
    ...
    probabilities_resized = cv2.resize(probabilities, (original_crop_shape[1], original_crop_shape[0]))
    return np.sum((probabilities_resized > seg_threshold).astype(np.uint8))

def wavelet_denoise(data, wavelet='db4', level=2):
    coeff = pywt.wavedec(data, wavelet, mode="per", level=level)
    sigma = (1 / 0.6745) * np.median(np.abs(coeff[-1]))
...
# --- Core Analysis (adapted from core.py for single video) ---
def run_analysis(video_path, original_filename, yolo_model, seg_model, config):
    ...
    for frame_count in range(total_frames):
        ...
        eye_box = (yolo_results[0].boxes[0].xyxy[0].cpu().numpy().astype(int)
                   if len(yolo_results) > 0 and len(yolo_results[0].boxes) > 0 else None)
        ...
        if eye_box is not None:
            ...
            if eye_crop.size > 0:
                ...
                current_area = postprocess_segmentation(seg_output, eye_crop.shape[:2], config['SEG_THRESHOLD'])
                status = 'processed'

        frame_data_list.append({'timestamp': timestamp, 'area': current_area, 'status': status})
    ...
    min_distance_frames = int(fps * 0.15)
    valley_search_frames = int(config['VALLEY_SEARCH_FRAMES'])

    # ==========================================================
    # Method 1: Interpolation (fill missing frames with linear interpolation)
    # ==========================================================
    ...
    raw_sig_interp = df['interp_area'].values
    denoised_sig_interp = wavelet_denoise(raw_sig_interp)
    roll_max_interp = (df['interp_area']
    ...
    # ==========================================================
    # Method 2: Zero method (keep missing frames as 0)
    # ==========================================================
    raw_sig_zero = df['area'].values
    denoised_sig_zero = wavelet_denoise(raw_sig_zero)
    roll_max_zero = (df['area']
                     .rolling(window=window_frames, center=True, min_periods=1)
                     .max().values)
    ...
    if len(valley_idx_interp) > 0:
        ...
        interp_avg_min_norm = local_baseline_norm(
            raw_sig_interp[valley_idx_interp], roll_max_interp[valley_idx_interp])
    else:
    ...
    if len(valley_idx_zero) > 0:
        ...
        zero_avg_min_norm = local_baseline_norm(
            raw_sig_zero[valley_idx_zero], roll_max_zero[valley_idx_zero])
    else:
    ...
    # ==========================================================
    # Visualization (fixed pixel baseline normalization)
    # ==========================================================
    raw_sig_interp_norm, interp_fixed_baseline = normalize_by_initial_baseline(
        raw_sig_interp, fps, config['BASELINE_WINDOW_SEC'])
    ...
    raw_sig_zero_norm, zero_fixed_baseline = normalize_by_initial_baseline(
        raw_sig_zero, fps, config['BASELINE_WINDOW_SEC'])
    ...
    # Method 1 subplot
    ax1.plot(df['timestamp'], raw_sig_interp_norm,
             label='Raw normalized (Interpolated)', color='lightgray', alpha=0.6)
    ax1.plot(df['timestamp'], roll_max_interp_norm,
             label='Baseline normalized', color='green', linestyle='--', alpha=0.7)
    ax1.plot(df['timestamp'], denoised_sig_interp_norm,
             label='Denoised normalized', color='blue', linewidth=1.5)
    ax1.axhline(1.0, color='black', linestyle=':', linewidth=1, alpha=0.6, label='Initial baseline = 1')

    if len(valley_idx_interp) > 0:
        ax1.scatter(df['timestamp'].iloc[valley_idx_interp],
                    raw_sig_interp_norm[valley_idx_interp],
                    color='red', s=80, zorder=5, label='Blink valley normalized')
    ...
    # Method 2 subplot
    ax2.plot(df['timestamp'], raw_sig_zero_norm,
             label='Raw normalized (Direct 0)', color='lightgray', alpha=0.6)
    ax2.plot(df['timestamp'], roll_max_zero_norm,
             label='Baseline normalized', color='green', linestyle='--', alpha=0.7)
    ax2.plot(df['timestamp'], denoised_sig_zero_norm,
             label='Denoised normalized', color='purple', linewidth=1.5)
    ax2.axhline(1.0, color='black', linestyle=':', linewidth=1, alpha=0.6, label='Initial baseline = 1')

    if len(valley_idx_zero) > 0:
        ax2.scatter(df['timestamp'].iloc[valley_idx_zero],
    ...
    stats = {
        ...
        "interp_blinks": len(peaks_interp),
        "interp_avg_min_area": round(interp_avg_min_area, 2),
        "interp_avg_min_norm": round(interp_avg_min_norm, 4),
        "interp_raw_avg_area": round(float(np.mean(raw_sig_interp)), 2),
        "interp_raw_norm_area": round(interp_raw_norm_area, 4),
        "interp_denoise_avg_area": round(float(np.mean(denoised_sig_interp)), 2),
        "interp_denoise_norm_area": round(interp_denoise_norm_area, 4),
        "zero_blinks": len(peaks_zero),
        "zero_avg_min_area": round(zero_avg_min_area, 2),
        "zero_avg_min_norm": round(zero_avg_min_norm, 4),
        "zero_raw_avg_area": round(float(np.mean(raw_sig_zero)), 2),
        "zero_raw_norm_area": round(zero_raw_norm_area, 4),
        "zero_denoise_avg_area": round(float(np.mean(denoised_sig_zero)), 2),
        "zero_denoise_norm_area": round(zero_denoise_norm_area, 4),
    }
...
def show_main_app():
    # Header
    st.markdown('<h1 class="main-header">👁 Mouse Blink Analysis</h1>', unsafe_allow_html=True)
    st.markdown('<p class="sub-header">Advanced signal processing for eye closure detection using wavelet transform and dynamic baseline analysis.</p>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        valley_search = st.slider(
            ...
            help="Frames to search backward for true blink valley after peak."
        )
    ...
    if start_button:
        ...
        if uploaded_file is not None:
            ...
            if yolo_model is None:
                st.error(f"YOLO model not found: {YOLO_MODEL_FILENAME}")
                return
            if seg_model is None:
                st.error(f"Segmentation model not found: {SEG_MODEL_FILENAME}")
                return
            ...
            video_bytes, results_df, results_fig, stats = run_analysis(
                video_path, uploaded_file.name, yolo_model, seg_model, config
            )
            ...
            if video_bytes or results_df is not None:
                ...
                st.error("Analysis failed. No eyes detected or video corrupted.")
    ...
    if st.session_state.analysis_results:
        ...
        with col4:
            ...
            blink_rate = (stats.get('interp_blinks', 0) / duration * 60) if duration > 0 else 0
            st.metric(
                "Blink Rate",
                f"{blink_rate:.1f}/min",
    ...
    if not history_df.empty:
        ...
        display_df.columns = [
            ...
            'Method1 Blinks', 'Method2 Blinks',
            'Interp Norm', 'Zero Norm', 'Duration'
        ]
